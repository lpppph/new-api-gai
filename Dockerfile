...
RUN cd web/default && bun install
COPY web/default ./web/default
RUN cd web/default && bun --smol run build

COPY web/classic/package.json web/classic/bun.lock ./web/classic/
RUN cd web/classic && bun install
COPY web/classic ./web/classic
RUN cd web/classic && VITE_REACT_APP_VERSION=$(cat ../../VERSION) bun run build
